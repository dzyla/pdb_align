...
new_save_pdb = """def save_aligned_pdb(self, filename: str, subset_only: bool = False):
        ...
        if chosen["seqguided"]:
            ...
            ref_atoms = [PseudoAtom(ref_infos[i].chain_id, ref_infos[i].resseq, ref_infos[i].icode) for (i, j) in pairs]
            mob_atoms = [PseudoAtom(mob_infos[j].chain_id, mob_infos[j].resseq, mob_infos[j].icode) for (i, j) in pairs]
        ...
        if R is not None and t is not None:
            ...
            dist_map = {}
            for k in range(len(mob_atoms)):
                ma = mob_atoms[k]
                key = (ma.chain_name, ma.res_seq, ma.res_icode)
